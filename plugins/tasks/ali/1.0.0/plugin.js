  ...
  openai_video: {
    decodeRequest: function (ctx) {
      ...
      if (ctx.body && ctx.body.kind === "json") req = ctx.body.value;
        ...
        if (
          (ctx.body.files || []).some(function (file) {
            return file.field === "input_reference";
          })
        )
          throw new Error("Ali requires input_reference to be a URL");
        const first = function (name) {
          const values = (ctx.body.fields || {})[name] || [];
          if (values.length > 1) throw new Error(name + " must be provided once");
          return values[0];
        };
        req = { prompt: first("prompt"), input_reference: first("input_reference"), seconds: first("seconds"), size: first("size") };
