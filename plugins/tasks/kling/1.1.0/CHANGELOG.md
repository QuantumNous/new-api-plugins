...
- Support Kling video generation through an upstream New API gateway on a type-60 channel, with the same plugin enabled on both gateways. Use gateway credentials and plugin-prefixed submission and query paths when the channel identifies a New API upstream, including tokens without the sk- prefix.
